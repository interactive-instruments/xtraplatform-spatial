...
public class SqlFeatureInsertsTest {
    ...
    public void testToSqlRef() {
        ...
        SqlPathTree parentPath = sqlPathTree.getChildren().get(1);
        SqlPathTree nestedPath = parentPath.getChildren().get(0);
        List<Integer> parentRows = ImmutableList.of(0, 0, 0);

        List<String> queries = inserts.toSqlRef(parentPath, nestedPath, parentRows)
                .stream()
                .map(query -> query.apply(VALUES2).first())
                .collect(Collectors.toList());

        LOGGER.debug("SQL \n{}", Joiner.on('\n')
                .join(queries));

        assertEquals(queries.get(0), EXPECTED.get(17));
    }
