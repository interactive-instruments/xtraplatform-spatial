...
public class TileMatrixSetRepositoryImpl implements TileMatrixSetRepository, AppLifeCycle {
  ...
  public TileMatrixSetRepositoryImpl(AppContext appContext) {
    this.customTileMatrixSetsStore =
        appContext.getDataDir().resolve(API_RESOURCES_DIR_NAME).resolve(TILE_MATRIX_SET_DIR_NAME);
  ...
  private void initCache() {
    ...
    Arrays.stream(
            Objects.requireNonNullElse(
                customTileMatrixSetsStore.toFile().listFiles(),
                ImmutableList.of().toArray(File[]::new)))
        ...
        .forEach(
            tileMatrixSetId ->
                TileMatrixSet.fromDefinition(tileMatrixSetId, customTileMatrixSetsStore)
                    .ifPresent(tms -> tileMatrixSets.put(tileMatrixSetId, tms)));
  }
}
