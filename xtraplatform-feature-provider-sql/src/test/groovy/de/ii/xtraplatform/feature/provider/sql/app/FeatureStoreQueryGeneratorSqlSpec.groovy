...
class FeatureStoreQueryGeneratorSqlSpec extends Specification {
    ...
    def 'Property with two nested filters'() {
        when:
        String metaQuery = queryGeneratorSql.getMetaQuery(FeatureStoreFixtures.MEASURE_CLASSIFICATION, 10, 0, Optional.of(CqlFilterExamples.EXAMPLE_33), Collections.emptyList(), true)
        List<String> instanceQueries = queryGeneratorSql.getInstanceQueries(FeatureStoreFixtures.MEASURE_CLASSIFICATION, Optional.of(CqlFilterExamples.EXAMPLE_33), Collections.emptyList(), null, null, Collections.emptyList(), Collections.emptyList()).collect(Collectors.toList())
        then:
