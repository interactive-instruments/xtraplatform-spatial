...
class FeatureStoreQueryGeneratorSqlSpec extends Specification {
    ...
    def 'ACONTAINS array operator'() {
        ...
        then:
        metaQuery == "SELECT * FROM (SELECT MIN(SKEY) AS minKey, MAX(SKEY) AS maxKey, count(*) AS numberReturned FROM (SELECT A.id AS SKEY FROM container A WHERE A.id IN (SELECT AA.id FROM container AA  WHERE AA.layers IN ('layers-ca','layers-us') GROUP BY AA.id HAVING count(*) = 2) ORDER BY SKEY LIMIT 10) AS NR) AS NR2, (SELECT count(*) AS numberMatched FROM (SELECT A.id AS SKEY FROM container A WHERE A.id IN (SELECT AA.id FROM container AA  WHERE AA.layers IN ('layers-ca','layers-us') GROUP BY AA.id HAVING count(*) = 2) ORDER BY 1) AS NM) AS NM2"
        instanceQuery == "SELECT A.id AS SKEY, A.layers FROM container A WHERE (A.id >= null AND A.id <= null) AND (A.id IN (SELECT AA.id FROM container AA  WHERE AA.layers IN ('layers-ca','layers-us') GROUP BY AA.id HAVING count(*) = 2)) ORDER BY 1"
    }
