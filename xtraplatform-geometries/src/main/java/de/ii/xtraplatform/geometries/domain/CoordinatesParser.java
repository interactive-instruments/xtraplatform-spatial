...
public class CoordinatesParser {
  ...
  public CoordinatesParser(
      SeperateStringsProcessor coordinatesProcessor, int sourceDimension, int targetDimension) {
  ...
  private void onChunkEnd(char[] chars, int offset, int length) throws IOException {
    if (length > 0) {
      if (hasChunkBoundaryBuffer()) {
        // chunkBoundaryBuffer = chunkBoundaryBuffer.concat(String.copyValueOf(chars, offset,
        // length));
        chunkBoundaryBuffer =
            ArrayUtils.addAll(
  ...
  private void onValue(char[] chars, int offset, int length) throws IOException {
    ...
    // if chunkBoundaryBuffer is not empty, we just started with a new input chunk and need to write
    // chunkBoundaryBuffer first
    if (hasChunkBoundaryBuffer()) {
