...
public abstract class CoordinatesTransformer extends Writer {
  ...
  protected Optional<DoubleArrayProcessor> getTransformationPipeline() {
    DoubleArrayProcessor lastTransformation =
        ImmutableToChars.of(getCoordinatesWriter(), getPrecision());
    ...
    if (getNormalizedMaxAllowableOffset() > 0.0) {
      lastTransformation =
          ImmutableSimplifyLine.of(
              lastTransformation, getNormalizedMaxAllowableOffset(), getMinNumberOfCoordinates());
      doTransform = true;
    }
  ...
  protected CoordinatesParser getCoordinatesParser() {
    return new CoordinatesParser(
        getCoordinatesProcessor(), getSourceDimension(), getTargetDimension());
  }
