...
public interface PropertySfFlat extends PropertyBase<PropertySfFlat, FeatureSchema> {
  ...
  default boolean hasCoordinates() {
    return getNestedProperties().stream()
        .anyMatch(child -> child.isCoordinate() || child.hasCoordinates());
  }
  ...
  default Optional<Geometry> getJtsPoint(GeometryFactory geometryFactory) {
    return getNestedProperties().isEmpty()
        ? Optional.empty()
        : getNestedProperties().get(0).getJtsCoordinate().map(geometryFactory::createPoint);
  }
  ...
  default Optional<LineString> getJtsLineString(
      GeometryFactory geometryFactory, boolean isTopLevel) {
  ...
  default Optional<MultiLineString> getJtsMultiLineString(GeometryFactory geometryFactory) {
    LineString[] lineStrings =
        getJtsGeometryArray(
            arrayProperty -> arrayProperty.getJtsLineString(geometryFactory, false),
            LineString[]::new);
  ...
  default Optional<Polygon> getJtsPolygon(GeometryFactory geometryFactory, boolean isTopLevel) {
    LinearRing[] linearRings =
        isTopLevel
            ? getJtsGeometryArray(
                arrayProperty -> arrayProperty.getJtsLinearRing(geometryFactory), LinearRing[]::new)
            : getJtsNestedGeometryArray(
                arrayProperty -> arrayProperty.getJtsLinearRing(geometryFactory),
                LinearRing[]::new);
        ...
        : linearRings.length == 1
            ...
            : Optional.of(
                geometryFactory.createPolygon(
                    linearRings[0], Arrays.copyOfRange(linearRings, 1, linearRings.length)));
  }

  default Optional<MultiPolygon> getJtsMultiPolygon(GeometryFactory geometryFactory) {
    Polygon[] polygons =
        getJtsGeometryArray(
            arrayProperty -> arrayProperty.getJtsPolygon(geometryFactory, false), Polygon[]::new);
    return polygons.length == 0
        ? Optional.empty()
        : Optional.of(geometryFactory.createMultiPolygon(polygons));
  }

  default <T extends Geometry> T[] getJtsGeometryArray(
      Function<PropertySfFlat, Optional<T>> geometryCreator, IntFunction<T[]> arrayCreator) {
    return getNestedProperties().isEmpty()
        ? arrayCreator.apply(0)
        : getNestedProperties().get(0).getJtsNestedGeometryArray(geometryCreator, arrayCreator);
  }

  default <T extends Geometry> T[] getJtsNestedGeometryArray(
      Function<PropertySfFlat, Optional<T>> geometryCreator, IntFunction<T[]> arrayCreator) {
    return getNestedProperties().stream()
        .map(geometryCreator)
  ...
  default <T extends Geometry> Optional<T> getJtsCoordinateArrayAsGeometry(
      Function<Coordinate[], T> geometryCreator, boolean isTopLevel) {
    Coordinate[] coordinateArray =
        isTopLevel ? getJtsCoordinateArray() : getJtsNestedCoordinateArray();
  ...
  default Coordinate[] getJtsCoordinateArray() {
    return getNestedProperties().isEmpty()
        ? new Coordinate[0]
        : getNestedProperties().get(0).getJtsNestedCoordinateArray();
  }
  ...
  default Optional<Coordinate> getJtsCoordinate() {
    ...
    return i >= 2 ? Optional.of(coordinate) : Optional.empty();
  }
}
