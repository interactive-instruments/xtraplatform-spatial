...
public class FeatureStoreQueryGeneratorSql implements FeatureStoreQueryGenerator<String> {
  ...
  public FeatureStoreQueryGeneratorSql(
      SqlDialect sqlDialect, EpsgCrs nativeCrs, CrsTransformerFactory crsTransformerFactory) {
    this.sqlDialect = sqlDialect;
    this.filterEncoder =
        new FilterEncoderSqlNewNewImpl(
            this::getAliases,
            (attributeContainer, aliases) ->
                (userFilterAttributeContainer, userFilter) ->
                    (instanceFilter) ->
                        getJoins(
                            attributeContainer,
                            userFilterAttributeContainer,
                            aliases,
                            userFilter,
                            instanceFilter),
            nativeCrs,
  ...
  public String getExtentQuery(
    ...
    String mainTable =
        String.format("%s %s", attributesContainer.getInstanceContainerName(), aliases.get(0));
            ...
            .map(
                attribute ->
                    sqlDialect.applyToExtent(
                        getQualifiedColumn(attributeContainerAlias, attribute.getName()), is3d))
            .get();
  ...
  public String getCountQuery(FeatureStoreInstanceContainer instanceContainer) {
    ...
    String mainTable = String.format("%s %s", instanceContainer.getName(), aliases.get(0));
  ...
  public String getTemporalExtentQuery(
    ...
    String column =
        attributesContainer
            .getTemporalAttribute(property)
            .map(
                attribute ->
                    sqlDialect.applyToDatetime(
                        getQualifiedColumn(attributeContainerAlias, attribute.getName())))
            .get();
    String join = getJoins(attributesContainer, aliases, Optional.empty());
    ...
    return String.format(
        ...
        column, column, table, join.isEmpty() ? "" : " ", join, where);
  }

  public String getTemporalExtentQuery(
      FeatureStoreInstanceContainer instanceContainer,
      FeatureStoreAttributesContainer startAttributesContainer,
    ...
    if (startAttributesContainer.equals(endAttributesContainer)) {
      ...
      String table =
          String.format(
              "%s %s", startAttributesContainer.getInstanceContainerName(), aliases.get(0));
      String startColumn =
          startAttributesContainer
              .getTemporalAttribute(startProperty)
              .map(
                  attribute ->
                      sqlDialect.applyToDatetime(
                          getQualifiedColumn(attributeContainerAlias, attribute.getName())))
              .get();
      String endColumn =
          endAttributesContainer
              .getTemporalAttribute(endProperty)
              .map(
                  attribute ->
                      sqlDialect.applyToDatetime(
                          getQualifiedColumn(attributeContainerAlias, attribute.getName())))
      ...
      return String.format(
          "SELECT MIN(%s), MAX(%s) FROM %s%s%s%s",
          startColumn, endColumn, table, join.isEmpty() ? "" : " ", join, where);
      ...
      String startTable =
          String.format(
              "%s %s", startAttributesContainer.getInstanceContainerName(), startAliases.get(0));
      ...
      String endTable =
          String.format(
              "%s %s", endAttributesContainer.getInstanceContainerName(), endAliases.get(0));
  ...
  private List<String> getAliases(FeatureStoreAttributesContainer attributeContainer) {
    ...
    FeatureStoreRelatedContainer relatedContainer =
        (FeatureStoreRelatedContainer) attributeContainer;
  ...
  private String getJoins(
      FeatureStoreAttributesContainer attributeContainer,
      List<String> aliases,
    ...
    return relatedContainer.getInstanceConnection().stream()
        .flatMap(
            relation ->
                toJoins(
                    relation,
  ...
  private String getJoins(
      FeatureStoreAttributesContainer attributeContainer,
      FeatureStoreAttributesContainer userFilterAttributeContainer,
      List<String> aliases,
      Optional<Cql2Expression> userFilter,
    ...
    FeatureStoreRelatedContainer relatedUserFilterContainer =
        (FeatureStoreRelatedContainer) userFilterAttributeContainer;
    String userFilterJoin =
        userFilter.isPresent()
            ? toJoins(
                    relatedUserFilterContainer.getInstanceConnection().get(0),
                    aliasesIterator,
                    getFilter(
                        userFilterAttributeContainer,
                        relatedUserFilterContainer.getInstanceConnection().get(0),
                ...
                .collect(Collectors.joining(" "))
            : "";
    String userFilterTargetField =
        userFilter.isPresent()
            ? relatedUserFilterContainer.getInstanceConnection().get(0).getTargetField()
            : "";

    FeatureStoreRelatedContainer relatedContainer =
        (FeatureStoreRelatedContainer) attributeContainer;
    String join =
        relatedContainer.getInstanceConnection().stream()
  ...
  private Optional<String> getFilter(FeatureStoreInstanceContainer instanceContainer) {
    ...
    return Optional.of(
        filterEncoder.encode(instanceContainer.getFilter().get(), instanceContainer));
  }

  private Optional<String> getFilter(
      FeatureStoreAttributesContainer attributesContainer,
      FeatureStoreRelation relation,
      Optional<Cql2Expression> cqlFilter) {
    if (relation.getFilter().isEmpty() && cqlFilter.isEmpty()) {
      return Optional.empty();
    }
    if (relation.getFilter().isPresent() && cqlFilter.isEmpty()) {
      return Optional.of(
          filterEncoder.encodeNested(relation.getFilter().get(), attributesContainer, false));
    }
    ...
    Cql2Expression mergedFilter = And.of(relation.getFilter().get(), cqlFilter.get());
