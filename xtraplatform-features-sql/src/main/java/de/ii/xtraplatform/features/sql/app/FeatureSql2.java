...
public interface FeatureSql2 {
  ...
  interface Object {
    ...
    default NestedObject getRelatedValues(
        List<FeatureStoreRelation> path, List<Integer> parentRows) {
      ...
      if (path.size() > 1) {
        ...
        return getNestedObjects()
            .get(path.subList(0, 1))
            .get(parentRows.get(0))
            .getRelatedValues(
                path.subList(1, path.size()), parentRows.subList(nestingLevel, parentRows.size()));
      }
    ...
    default Object getCurrentRelatedValues(List<FeatureStoreRelation> path) {
      ...
      int row = Optional.ofNullable(getNestedObjects().get(path)).map(List::size).orElse(0);
