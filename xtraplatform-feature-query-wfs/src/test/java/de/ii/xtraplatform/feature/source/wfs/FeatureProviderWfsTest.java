        ...
        new TestKit(system) {
            {
                ...
                FeatureTypeConfiguration featureType = new FeatureTypeConfiguration("AX_Bahnstrecke", "http://www.adv-online.de/namespaces/adv/gid/6.0", "adv:AX_Bahnstrecke");
                AkkaHttp akkaHttp = new AkkaHttpMock(ActorMaterializer.create(probe.getSystem()), ByteString.fromString(doc));

                GmlProvider provider = new FeatureProviderWfs(akkaHttp, wfs, ImmutableMap.<String, FeatureTypeConfiguration>builder().put("AX_Bahnstrecke", featureType)
                                                                                                                                     .build());
                ...
                FeatureStream<GmlConsumer> featureStream = provider.getFeatureStream(query, akkaHttp);
                CompletionStage<Done> done = featureStream.apply(new GmlConsumer() {
                    ...
                    @Override
                    public void onGmlPropertyEnd(String namespaceURI, String localName, List<String> path) throws Exception {
