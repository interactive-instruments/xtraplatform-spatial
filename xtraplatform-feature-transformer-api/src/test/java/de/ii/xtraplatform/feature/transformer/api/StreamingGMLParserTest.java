 ...
 * @author zahnen
 */
public class StreamingGMLParserTest {
    ...
    public void testParser() throws Exception {
        ...
        final Flow<ByteString, EventBasedStreamingGmlParser.GmlEvent, NotUsed> parser = EventBasedStreamingGmlParser.parser(new QName("http://www.adv-online.de/namespaces/adv/gid/6.0", "AX_Bahnstrecke"));

        final Sink<String, CompletionStage<List<EventBasedStreamingGmlParser.GmlEvent>>> parse = Flow.<String>create()
                .map(ByteString::fromString)
                .via(parser)
                .toMat(Sink.seq(), Keep.right());

        final Sink<String, CompletionStage<Done>> parse2 = Flow.<String>create()
                .map(ByteString::fromString)
                .via(parser)
                .toMat(Sink.foreach(event -> System.out.println(event.toString())), Keep.right());

        final Sink<String, CompletionStage<Done>> parse3 = Flow.<String>create()
                .map(ByteString::fromString)
                .via(XmlParsing.parser())
                .toMat(Sink.foreach(event -> System.out.println(event.toString())), Keep.right());

        CompletionStage<HttpResponse> httpResponseCompletionStage = Http.get(system).singleRequest(HttpRequest.GET("http://www.google.de"));
        CompletionStage<HttpResponse> voidCompletionStage = httpResponseCompletionStage.whenComplete((httpResponse, throwable) -> {
            System.out.println("RESPONSE");
            System.out.println(httpResponse.status());
            //httpResponse.entity().getDataBytes().via(parser).runWith(Sink.foreach(event -> System.out.println(event.toString())), materializer);
        });
        voidCompletionStage.thenRun(() -> System.out.println("DONE"));

        //final CompletionStage<List<GmlEvent>> resultStage = Source.single(doc).runWith(parse, materializer);
        //final CompletionStage<Done> resultStage2 = Source.single(doc).runWith(parse2, materializer);
        //resultStage2.thenRun(() -> System.out.println("DONE"));
        ...
        new TestKit(system) {
            {
                ...
                String response = probe.expectMsgClass(duration("10 seconds"), String.class);
