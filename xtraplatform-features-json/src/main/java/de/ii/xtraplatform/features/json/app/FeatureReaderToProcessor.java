...
// TODO: FeatureMapper
public class FeatureReaderToProcessor<
        T extends PropertyBase<T, V>, U extends FeatureBase<T, V>, V extends SchemaBase<V>>
    implements FeatureReader<V, V> {
  ...
  @Override
  public void onStart(OptionalLong numberReturned, OptionalLong numberMatched, V context)
      throws Exception {
    if (numberReturned.isPresent() || numberMatched.isPresent()) {
      this.collectionMetadata =
          ModifiableCollectionMetadata.create()
  ...
  private T createProperty(Property.Type type, List<String> path, V schema, String value) {
    ...
    return currentFeature.getProperties().stream()
        .filter(
            t ->
                t.getType() == type
                    && t.getSchema().isPresent()
                    && Objects.equals(t.getSchema().get(), schema)
                    && !t.getSchema().get().isSpatial())
        .findFirst()
        .orElseGet(
            () -> {
              T property = featureProcessor.createProperty();
              // TODO: multiple properties for same path
              property.type(type).schema(schema);
  ...
  private T getCurrentParent() {
    return Objects.nonNull(currentProperty) && currentProperty.getParent().isPresent()
        ? currentProperty.getParent().get()
        : null;
  }
}
